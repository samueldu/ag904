		...
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					if(isset($product_query->row['manufacturer_id']))
					{
						$query_manu = $this->db->query("SELECT * FROM " . DB_PREFIX . "manufacturer m LEFT JOIN " . DB_PREFIX . "manufacturer_to_store m2s ON (m.manufacturer_id = m2s.manufacturer_id) WHERE m.manufacturer_id = '" . (int)$product_query->row['manufacturer_id'] . "' AND m2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");

						$manu = $query_manu->row['name'];
