...
class ControllerUserUser extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
            ...
            if($this->user->getId() == 2 and $result['user_id'] == 2)
            {
	...
	protected function getForm() {
		...
		if (isset($this->request->get['user_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$user_info = $this->model_user_user->getUser($this->request->get['user_id'],$this->session->data['login_core']);
		}
        ...
        foreach($data['user_groups'] as $key=>$value)
        {
            if($this->user->getGroupId() != 1 and $data['user_groups'][$key]['user_group_id'] == 1)
                unset($data['user_groups'][$key]);
        }
