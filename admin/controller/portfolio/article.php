<?php
	class ControllerPortfolioArticle extends Controller {
		private $error = array();
		...
		public function insert() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
				...
				$this->model_portfolio_article->addArticle($this->request->post);
				...
				$this->response->redirect($this->url->link('portfolio/article', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			}
		...
		public function update() {
			...
			if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateForm())) {
				//print "<pre>"; print_r($this->request->post); exit;
				$this->model_portfolio_article->editArticle($this->request->get['portfolio_article_id'], $this->request->post);
		...
		public function delete() {
			...
			if (isset($this->request->post['selected']) && $this->validateDelete()) {
				...
				foreach ($this->request->post['selected'] as $portfolio_article_id) {
					$this->model_portfolio_article->deleteArticle($portfolio_article_id);
				}
		...
		public function getList() {
			...
			$data['breadcrumbs'] = array();
	   		...
	   		$data['breadcrumbs'][] = array(
				...
				'href'      => $this->url->link('portfolio/article', 'token=' . $this->session->data['token'] . $url, 'SSL'),
	      		'separator' => ' :: '
	   		);
			
			$data['insert'] = $this->url->link('portfolio/article/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
			$data['delete'] = $this->url->link('portfolio/article/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
			...
			$article_limit = $this->model_portfolio_article->getTotalArticle($filter_data);
			
			$results = $this->model_portfolio_article->getArticles($filter_data);
			...
			foreach($results as $result) {
				$data['articles'][] = array(
					...
					'selected'        	=> isset($this->request->post['selected']) && in_array($result['portfolio_article_id'], $this->request->post['selected']),
					'edit'          	=> $this->url->link('portfolio/article/update', 'token=' . $this->session->data['token'] . '&portfolio_article_id=' . $result['portfolio_article_id'] . $url, 'SSL')
				);		
			}
			...
			$data['sort_article_title'] = $this->url->link('portfolio/article', 'token=' . $this->session->data['token'] . '&sort=sbad.article_title' . $url, 'SSL');
			$data['sort_author_name'] = $this->url->link('portfolio/article', 'token=' . $this->session->data['token'] . '&sort=sbau.name' . $url, 'SSL');
			$data['sort_sortorder'] = $this->url->link('portfolio/article', 'token=' . $this->session->data['token'] . '&sort=sba.sort_order' . $url, 'SSL');
			$data['sort_status'] = $this->url->link('portfolio/article', 'token=' . $this->session->data['token'] . '&sort=sba.status' . $url, 'SSL');
			$data['sort_date_added'] = $this->url->link('portfolio/article', 'token=' . $this->session->data['token'] . '&sort=sba.date_added' . $url, 'SSL');
			...
			$pagination->url = $this->url->link('portfolio/article', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
    		...
    		$this->response->setOutput($this->load->view('portfolio/article_list.tpl', $data));
		}		

		public function getForm() {
			$data['heading_title'] = $this->language->get('heading_title');
			
            $data['text_form'] = !isset($this->request->get['portfolio_article_id']) ? $this->language->get('text_add') : $this->language->get('text_edit');
            ...
            $data['tab_frases'] = $this->language->get('tab_frases');
			
			$data['token'] = $this->session->data['token'];
			
			if (isset($this->request->get['portfolio_article_id'])) {
				$data['portfolio_article_id'] = $this->request->get['portfolio_article_id'];
			} else {
				$data['portfolio_article_id'] = 0;
			}
			...
			if (!isset($this->request->get['portfolio_article_id'])) {
				$data['action'] = $this->url->link('portfolio/article/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
			} else {
				$data['action'] = $this->url->link('portfolio/article/update', 'token=' . $this->session->data['token'] . '&portfolio_article_id=' . $this->request->get['portfolio_article_id'] . $url, 'SSL');
			}

			$data['cancel'] = $this->url->link('portfolio/article', 'token=' . $this->session->data['token'] . $url, 'SSL');
		
			if ((isset($this->request->get['portfolio_article_id'])) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
				$article_info = $this->model_portfolio_article->getArticle($this->request->get['portfolio_article_id']);
			}

			$this->load->model('portfolio/author');
			$data['authors'] = array();
			$data['authors'] = $this->model_portfolio_author->getAuthors();
			...
			if (isset($this->request->post['article_description'])) {
				...
				$data['article_description'] = $this->model_portfolio_article->getArticleDescriptions($this->request->get['portfolio_article_id']);
			} else {
			...
			if (isset($this->request->post['article_addition_description'])) {
				...
				$data['article_addition_description'] = $this->model_portfolio_article->getArticleAdditionalDescriptions($this->request->get['portfolio_article_id']);
			} else {
			...
			if (isset($this->request->post['portfolio_author_id'])) {
				$data['author_name'] = $this->request->post['author_name'];
				$data['portfolio_author_id'] = $this->request->post['portfolio_author_id'];
			} elseif (isset($article_info)) {
				$data['portfolio_author_id'] = $article_info['portfolio_author_id'];
				$data['author_name'] = $this->model_portfolio_author->getAuthorName($article_info['portfolio_author_id']);
			} else {
				$data['author_name'] = '';
				$data['portfolio_author_id'] = '';
			}
			...
			if (isset($this->request->post['article_store'])) {
				...
				$data['article_store'] = $this->model_portfolio_article->getArticleStore($this->request->get['portfolio_article_id']);
			} else {
			...
			if (isset($this->request->post['article_category'])) {
				...
				$data['article_category'] = $this->model_portfolio_article->getArticleCategories($this->request->get['portfolio_article_id']);
			} else {
			...
			if (isset($this->request->post['related_article'])) {
				...
				if($article_info['article_related_method']) {
					...
					if($data['related_article'] == 'category_wise' && $options) {
						...
						$products = $this->model_portfolio_article->getArticleProduct($this->request->get['portfolio_article_id']);
			...
			if (isset($this->request->post['blog_related_articles'])) {
				...
				$data['blog_related_articles'] = $this->model_portfolio_article->getRelatedArticles($this->request->get['portfolio_article_id']);
			} else {
			...
			if (isset($this->request->post['article_layout'])) {
				...
				$data['article_layout'] = $this->model_portfolio_article->getArticleLayouts($this->request->get['portfolio_article_id']);
			} else {
            ...
            if (isset($this->request->post['gallery_id'])) {
                ...
                $data['gallery_id'] = $article_info['gallery_id'];
            } else {
                $data['gallery_id'] = '';
            }
            ...
            $this->response->setOutput($this->load->view('portfolio/article_form.tpl', $data));
		}
		...
		public function autocomplete() {
			...
			if (isset($this->request->get['article_name'])) {
                ...
                if($article_name) {
    				...
    				$results = $this->model_portfolio_article->getArticles($data);
		...
		private function validateForm() {
			if (!$this->user->hasPermission('modify', 'portfolio/article')) {
			...
			foreach ($this->request->post['article_description'] as $language_id => $value) {
				if ((strlen($value['article_title']) < 3) || (strlen($value['article_title']) > 100)) {
					...
					if(!isset($this->request->get['portfolio_article_id'])) {
						$found = $this->model_portfolio_article->checkArticleName($language_id, $value['article_title'], 0);
						...
						$found = $this->model_portfolio_article->checkArticleName($language_id, $value['article_title'], $this->request->get['portfolio_article_id']);
						if($found) {
							$this->error['warning'] = $this->language->get('error_title_found');	
							$this->error['article_title'][$language_id] = $this->language->get('error_title_found');
						}	
			...
			if(!$this->request->post['author_name']) {
				...
				if($this->request->post['portfolio_author_id']) {
					$found = $this->model_portfolio_article->checkAuthorName($this->request->post['author_name']);
		...
		private function validateDelete() {
			...
			foreach ($this->request->post['selected'] as $portfolio_article_id) {
				$found = $this->model_portfolio_article->checkDeleteArticle($portfolio_article_id);
		...
		public function autocomplete_article() {
			...
			if(isset($this->request->get['portfolio_article_id'])) {
                ...
                if($filter_name) {
    				...
    				$results = $this->model_portfolio_article->getArticlesRelated($filter_data, $this->request->get['portfolio_article_id']);
