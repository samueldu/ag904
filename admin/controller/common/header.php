...
class ControllerCommonHeader extends Controller {
	public function index() {
		...
		if (!isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
			...
			$data['logo'] = $this->model_tool_image->resizeExternal($this->config->get('config_logo'), 100, 30);
            ...
            if($this->user->getGroupId() == 1)
            {
                ...
                $results = $this->model_common_header->getAllClients();
				if($results->num_rows)
                foreach ($results->rows as $result) {
                    $data['clients'][] = array(
                        'name' => $result['config_name'],
                        'id' => $result['id'],
						'img' => $this->model_tool_image->resizeExternal($result['config_logo'], 50, 50),
                        'href' => $result[$var.'url']
                    );
                }
            }
