...
class ModelUserUser extends Model {
	...
	public function getActiveUser($user_id,$loginCore)
	{
		if ($loginCore == 0)
		{
			$query = $this->db->query("SELECT *, (SELECT ug.name FROM `" . DB_PREFIX . "user_group` ug WHERE ug.user_group_id = u.user_group_id) AS user_group FROM `" . DB_PREFIX . "user` u WHERE u.user_id = '" . (int)$user_id . "'");
		}
		elseif ($loginCore == 1 or $this->user->getGroupId()==1)
		{
			$query = $this->dbcore->query("SELECT *, (SELECT ug.name FROM `" . DB_PREFIX_CORE . "user_group` ug WHERE ug.user_group_id = u.user_group_id) AS user_group FROM `" . DB_PREFIX_CORE . "user` u WHERE u.user_id = '" . (int)$user_id . "'");
		}
		return $query->row;
	}
