...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST' and $this->validate())) {
			...
			foreach ($custom_fields as $custom_field) {
				if ($custom_field['location'] == 'contact') {
					if (!is_array($this->request->post['custom_field'][$custom_field['custom_field_id']])) {
						$body .= $custom_field['name'] . ": " . $this->request->post['custom_field'][$custom_field['custom_field_id']] . "\n";
					} else {
						//foreach($this->request->post['custom_field'][$custom_field['custom_field_id']] as $key=>$value)
						//{
							foreach($custom_fields as $keyx=>$valuex)
							{
								if($custom_fields[$keyx]['custom_field_id'] == $custom_field['custom_field_id']) {

								foreach($custom_fields[$keyx]['custom_field_value'] as $keyy=>$valuey)
								{
									foreach($this->request->post['custom_field'][$custom_field['custom_field_id']] as $chave=>$valor)

									if($custom_fields[$keyx]['custom_field_value'][$keyy]['custom_field_value_id'] == $valor)
									{
										$body .= $custom_field['name']." : ".$custom_fields[$keyx]['custom_field_value'][$keyy]['name']."\n";
									}
								}
							}
						}
		...
		$data['error_parsley_name'] = $this->language->get('error_parsley_name');
		$data['error_parsley_email'] = $this->language->get('error_parsley_email');
		$data['error_parsley_enquiry'] = $this->language->get('error_parsley_enquiry');
