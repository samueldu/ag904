	...
	class ControllerPortfolioAuthor extends Controller {
		public function index() {
			...
			if(isset($this->request->get['portfolio_author_id'])) {
				$portfolio_author_id = $this->request->get['portfolio_author_id'];
			} else {
				$portfolio_author_id = 0;
			}
			...
			if($portfolio_author_id) {
				...
				$author_total = $this->model_portfolio_article->getTotalArticleAuthorWise($portfolio_author_id);
				
				$results = $this->model_portfolio_article->getArticleAuthorWise($filter_data);
				...
				if($data['articles']) {					
					...
					$author_info = $this->model_portfolio_article->getAuthorInformation($portfolio_author_id);
				...
				$pagination->url = $this->url->link('portfolio/author', 'portfolio_author_id=' . $this->request->get['portfolio_author_id'] . '&page={page}');
				...
				if (isset($this->request->get['portfolio_author_id'])) {
					$url .= '&portfolio_author_id=' . $this->request->get['portfolio_author_id'];
				}
