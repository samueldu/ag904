...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
            ...
            {
                $aux = $this->model_catalog_category->getCategory($categories[$key]['category_id']);

                if($aux['parent_id'] != 0)
                {
                    $data['categories'][$aux['parent_id']]['sub'][] = $aux;

                }
                else
                {
                    $data['categories'][$aux['category_id']] = $aux;
                }
            }
			...
			$data['parcelamento'] = array();
			$data['parcelamento'] = $this->load->controller('product/product/parcelamento',$product_info);

			$data['badges'] = array();
			$data['badges'] = $this->load->controller('product/product/badges',$product_info);
			...
			foreach ($results as $result) {
				...
				foreach($categories as $key=>$value)
				{
					...
					if($aux['parent_id'] != 0)
					{
						$catAux[$aux['parent_id']]['sub'][] = $aux;

					}
					else
					{
						$catAux[$aux['category_id']] = $aux;
					}
				}
				...
				$data['products'][] = array(
					...
					'badges'      => $this->load->controller('product/product/badges',$result['product_id']),
	...
	public function badges($product)
	{
		...
		$badges = $this->model_catalog_product->getBadge($id);

		foreach($badges as $key=>$value)
		{
			$badges[$key]['text_clean'] = $this->clean($badges[$key]['text']);
		}
	...
	public function parcelamento($product)
	{

		$this->load->language('product/product');

		$qtd_parcelas = $this->config->get('config_parcelamento_nparcelas');
		$juros = $this->config->get('config_parcelamento_juros');
		$moeda_da_loja = 'R$';
		$tipo_de_calculo = 0;
		$parcela_minima = $this->config->get('config_parcelamento_minima');

		if (!$product['special']) {
			$preco_numero = ($product['price']);
		} else {
			$preco_numero = ($product['special']);
		}
		...
		$format = $this->language->get("text_parcelamento");

		$var['texto'] = sprintf($format, $qtd_parcelas, $moeda_da_loja . number_format($valor_parcela, 2, ',', '.'));

		if ($juros == 0)
		{
			$var['juros'] = $this->language->get("text_no_juros");
		}
		else
		{
			$var['juros'] = sprintf($this->language->get("text_com_juros"),$juros);
		}
