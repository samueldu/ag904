<?php
	class ControllerSimpleBlogArticle extends Controller {
		public function index() {
			...
			$this->document->setDescription($this->config->get('simple_blog_meta_desc'));

			$this->document->setKeywords($this->config->get('simple_blog_meta_key'));
			
			$results = $this->model_simple_blog_article->getCategories(0);
			
			foreach ($results as $result) {

				$data['categories'][] = array(
					'name'  => $result['name'],
					'href'  => $this->url->link('simple_blog/category', 'simple_blog_category_id=' . $result['simple_blog_category_id'])
				);						
			}
			...
			if (isset($this->request->get['limit'])) {
				...
				$limit =  $this->config->get('simple_blog_limit');
			}
			...
			foreach($results as $result) {
				...
				foreach($match[0] as $key=>$value) {
					$description = str_replace($match[0][$key], "", $description);
				}
				
				$description = utf8_substr(strip_tags(html_entity_decode($description, ENT_QUOTES, 'UTF-8')), 0, 300) . '...';
				...
				$data['articles'][] = array(
					...
					'category'     		=> $this->model_simple_blog_article->getCategorieByArticle($result['simple_blog_article_id'])
		...
		public function view() {
			...
			if($simple_blog_article_id) {
				...
				$data['next_article'] = $this->model_simple_blog_article->getNext($article_info['date_added']);

				$data['next_prev'] = array();

				foreach($data['next_article'] as $key=>$value)
				{
					$data['next_prev'][$data['next_article'][$key]['direction']] = $this->url->link('simple_blog/article/view', 'simple_blog_article_id=' . $data['next_article'][$key]['simple_blog_article_id']);
				}
				...
				if($article_info) {
					...
					$data['article_date_modified_long'] = strftime($this->language->get('text_date_format_long'), strtotime($article_info['date_modified']));
					...
					$data['category'] = $this->model_simple_blog_article->getCategorieByArticle($simple_blog_article_id);
					...
					if($author_info) {
						...
						if($author_info['image']) {
							$data['author_image'] = $this->model_tool_image->resize($author_info['image'], 250, 250);
						} else {
							$data['author_image'] = $this->model_tool_image->resize('no_image.jpg', 250, 250);
						}
                    ...
                    foreach($related_articles as $article) {
                        $data['related_articles'][] = array(
    						...
    						'date_added'		=> date($this->language->get('text_date_format'), strtotime($article['date_added'])),
							'date_added_long'	=> strftime($this->language->get('text_date_format_long'), strtotime($result['date_added'])),
    						'date_modified'		=> date($this->language->get('text_date_format'), strtotime($article['date_modified'])),
					...
					/* modifica as galerias */
					preg_match_all('#\{(.*?)\}#', $article_info['description'], $match);
					...
					foreach($match[0] as $key=>$value)
					{
                        ...
                        if(substr_count($match[0][$key],"galeria:"))
                        {
                            $aux = explode(",",$match[0][$key]);
                            $id = str_replace("{galeria:","",$aux[0]);
                            $coluna = trim(str_replace(array("colunas:","}"),"",$aux[1]));

                            $texto = '<div class="gallery gallery-columns-'.$coluna.'">';

                            $results = $this->model_design_banner->getBanner($id);

                            $name = $this->model_design_banner->getBannerName($id);

                            foreach ($results as $result) {
                            //	if (is_file(DIR_IMAGE . $result['image'])) {
                                    $texto .= ' <a href="'.HTTP_IMAGE.$result['image'].'" data-lightbox-gallery="'.$name.'" class="gallery-item" itemprop="contentUrl"><img itemprop="thumbnail" src="'.HTTP_IMAGE.$result['image'].'" alt="'.$result['title'].' | NOME DO SITE"></a>';
                            //	}
                            }

                            $texto .= '</div>';

                            $article_info['description'] = str_replace($match[0][$key],utf8_decode($texto),$article_info['description']);
                        }
					}
				}

				$data['article_info']['description'] = $article_info['description'];
				...
				$data['breadcrumbs'][] = array(
					'text'      => $data['article_info']['article_title'],
					'href'      => $this->url->link('simple_blog/article/view', 'simple_blog_article_id=' . $data['article_info']['simple_blog_article_id'], 'SSL'),
					'separator' => ' :: '
				);
		...
		public function comment() {
			...
			$results = $this->model_simple_blog_article->getCommentsByArticle($this->request->get['simple_blog_article_id'],($page - 1) * $limit, $limit, 0);
			...
			foreach ($results as $result) {
                ...
                foreach($comment_reply as $comment) {
                    $comment_replies[] = array(
                        ...
                        'date_added'    => strftime($this->language->get('text_date_format_long'), strtotime($comment['date_added'])),
				...
				$data['comments'][] = array(
					...
					'date_added' 		=> strftime($this->language->get('text_date_format_long'), strtotime($result['date_added']))
				);
			}
