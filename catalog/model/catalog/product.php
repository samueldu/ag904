...
(SELECT m.name FROM " . DB_PREFIX . "manufacturer m where (p.manufacturer_id = m.manufacturer_id)) as manufacturer";
	...
	public function getBadge($product_id) {
		return $this->db->query("SELECT * FROM `" . DB_PREFIX . "product_to_badge`
		inner join `" . DB_PREFIX . "badge` on `" . DB_PREFIX . "product_to_badge`.badge_id = `" . DB_PREFIX . "badge`.badge_id
		WHERE `" . DB_PREFIX . "product_to_badge`.product_id = '" . (int)$product_id."'")->rows;
	}
}
