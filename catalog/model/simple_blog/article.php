<?php
	class ModelSimpleBlogArticle extends Model {
		...
		{
			$sql = "
			SELECT 'next' AS direction, MIN(simple_blog_article_id) AS simple_blog_article_id, MIN(date_added) AS date_key
			FROM `" . DB_PREFIX . "simple_blog_article` 
			  WHERE date_added > '$date'  and `status`= '1'
				GROUP BY 1
				UNION
			SELECT 'prev' AS direction, MAX(simple_blog_article_id) AS simple_blog_article_id, MAX(date_added) AS date_key
  			FROM `" . DB_PREFIX . "simple_blog_article`
  			WHERE date_added < '$date'  and `status`= '1'
			GROUP BY 1
			ORDER BY 1 DESC";
		...
		public function getCategorieByArticle($article_id)
		{
			...
			$sql = $this->db->query("SELECT * FROM `" . DB_PREFIX . "simple_blog_category` sbc 
			INNER JOIN `" . DB_PREFIX . "simple_blog_category_description` sbcd ON(sbc.simple_blog_category_id=sbcd.simple_blog_category_id)
			INNER JOIN `" . DB_PREFIX . "simple_blog_article_to_category` sbac ON(sbc.simple_blog_category_id=sbac.simple_blog_category_id) and sbac.simple_blog_article_id = '".$article_id."'
			INNER JOIN `" . DB_PREFIX . "simple_blog_category_to_store` sbcs ON(sbc.simple_blog_category_id=sbcs.simple_blog_category_id)
